...
function createSessionId() {
    return `leo-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`
}
...
export default function ThreadsPage() {
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const reply = await sendLeoMessage(message, history, sessionIdRef.current)
    ...
    return (
        <main className="mx-auto my-8 w-full max-w-4xl px-4 text-left font-sans">
            <h1 className="text-center">Chat with Leo</h1>
            <ModuleNavigation/>

            <section
                className="mb-6 rounded-xl border border-violet-200 bg-violet-50/70 p-4 text-sm text-violet-950 dark:border-violet-900/60 dark:bg-violet-950/20 dark:text-violet-100">
                ...
                <p className="mt-1 text-violet-900/80 dark:text-violet-100/80">
                    ...
                    conversation history and sends it to the configured Leo webhook.
                </p>
            </section>
            ...
            <section
                ...
                <form onSubmit={handleSubmit} className="grid gap-3 p-4">
                    <label className="grid gap-1 text-sm font-medium" htmlFor="leo-message">
                        ...
                        <textarea
                            id="leo-message"
                            className="min-h-32 rounded-md border border-gray-300 bg-white px-3 py-2 text-base text-gray-900 shadow-sm outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-200 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:focus:border-violet-400 dark:focus:ring-violet-900/40"
                            value={draft}
