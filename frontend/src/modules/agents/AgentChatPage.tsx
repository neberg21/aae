import { useRef, useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import ModuleNavigation from './ModuleNavigation'
import { sendAgentChatMessage, getAgent } from './api'
import type { ChatMessage, ThreadMessage } from './types'
import { useEffect } from 'react'

type Status = 'idle' | 'sending' | 'error' | 'loading'

export default function AgentChatPage() {
  const { agentId } = useParams<{ agentId: string }>()
  const navigate = useNavigate()

  const [agentName, setAgentName] = useState<string>('')
  const [messages, setMessages] = useState<ChatMessage[]>([])
  const [draft, setDraft] = useState('')
  const [status, setStatus] = useState<Status>('loading')
  const [error, setError] = useState<string | null>(null)
  const threadIdRef = useRef<string | null>(null)

  useEffect(() => {
    if (!agentId) {
      navigate('/module/agents/list')
      return
    }

    // Load agent details to display the name
    async function loadAgent(id: string) {
      try {
        const agent = await getAgent(id)
        setAgentName(agent.name)
        setStatus('idle')
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to load agent')
        setStatus('error')
      }
    }

    void loadAgent(agentId)
  }, [agentId, navigate])

  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()

    if (!agentId) return

    const message = draft.trim()
    if (!message || status === 'sending') {
      return
    }

    setDraft('')
    setError(null)
    setStatus('sending')

    try {
      const result = await sendAgentChatMessage(
        agentId,
        message,
        threadIdRef.current ?? undefined
      )
      threadIdRef.current = result.threadId

      // Convert chatMessages from API format to UI format
      const convertedMessages: ChatMessage[] = result.chatMessages.map((msg: ThreadMessage) => ({
        role: msg.sender === 'user' || msg.sender === 'You' ? 'user' : 'assistant',
        content: msg.content,
      }))

      // Replace ALL messages with the ones from the API
      setMessages(convertedMessages)
      setStatus('idle')
    } catch (err) {
      setStatus('error')
      setError(err instanceof Error ? err.message : 'Failed to send message')
    }
  }

  function handleReset() {
    threadIdRef.current = null
    setMessages([])
    setDraft('')
    setError(null)
    setStatus('idle')
  }

  if (status === 'loading') {
    return (
      <main className="mx-auto my-8 w-full max-w-4xl px-4 text-left font-sans">
        <h1 className="text-center">Chat with Agent</h1>
        <ModuleNavigation />
        <p className="text-sm text-gray-600 dark:text-gray-300">Loading...</p>
      </main>
    )
  }

  if (!agentId) {
    return null
  }

  return (
    <main className="mx-auto my-8 w-full max-w-4xl px-4 text-left font-sans">
      <h1 className="text-center">Chat with {agentName}</h1>
      <ModuleNavigation />

      <section className="mb-6 rounded-xl border border-blue-200 bg-blue-50/70 p-4 text-sm text-blue-950 dark:border-blue-900/60 dark:bg-blue-950/20 dark:text-blue-100">
        <p className="font-semibold">{agentName} is available.</p>
        <p className="mt-1 text-blue-900/80 dark:text-blue-100/80">
          Chat directly with this agent to ask questions or request assistance.
        </p>
      </section>

      <section className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <div
          role="log"
          aria-live="polite"
          aria-label={`${agentName} conversation`}
          className="grid gap-4 border-b border-gray-200 p-4 dark:border-gray-700 min-h-64"
        >
          {messages.length === 0 && (
            <p className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-800/50 dark:text-gray-300">
              Start a conversation with {agentName}.
            </p>
          )}

          {messages.map((message, index) => (
            <article
              key={`${message.role}-${index}`}
              className={
                message.role === 'user'
                  ? 'ml-auto max-w-[85%] rounded-2xl rounded-br-md bg-violet-600 px-4 py-3 text-sm text-white'
                  : 'mr-auto max-w-[85%] rounded-2xl rounded-bl-md bg-gray-100 px-4 py-3 text-sm text-gray-900 dark:bg-gray-800 dark:text-gray-100'
              }
            >
              <p className="mb-1 text-xs font-semibold uppercase tracking-wide opacity-70">
                {message.role === 'user' ? 'You' : agentName}
              </p>
              <p className="whitespace-pre-wrap">{message.content}</p>
            </article>
          ))}

          {status === 'sending' && (
            <p className="text-sm text-gray-600 dark:text-gray-300">{agentName} is thinking...</p>
          )}
        </div>

        <form onSubmit={handleSubmit} className="grid gap-3 p-4">
          <label className="grid gap-1 text-sm font-medium" htmlFor="agent-message">
            Your message
            <textarea
              id="agent-message"
              className="min-h-32 rounded-md border border-gray-300 bg-white px-3 py-2 text-base text-gray-900 shadow-sm outline-none transition disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100"
              value={draft}
              onChange={(event) => setDraft(event.target.value)}
              placeholder={`Write to ${agentName}...`}
              disabled={status === 'sending'}
            />
          </label>

          {error && <p role="alert" className="text-sm text-red-600 dark:text-red-400">{error}</p>}

          <div className="flex flex-wrap gap-3">
            <button
              type="submit"
              disabled={status === 'sending' || draft.trim().length === 0}
              className="rounded-md bg-violet-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-60"
            >
              Send
            </button>
            <button
              type="button"
              onClick={handleReset}
              className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-800 transition hover:bg-gray-100 dark:border-gray-700 dark:text-gray-100 dark:hover:bg-gray-800"
            >
              New conversation
            </button>
            <button
              type="button"
              onClick={() => navigate('/module/agents/list')}
              className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-800 transition hover:bg-gray-100 dark:border-gray-700 dark:text-gray-100 dark:hover:bg-gray-800"
            >
              Back to agents
            </button>
          </div>
        </form>
      </section>
    </main>
  )
}



