...
**Think workflow callbacks / identity reads:** hardcoded base `https://ai.neberg.de` in the imported JSON (for example `POST /api/agents/route-chat-message`, `POST /api/agents/create-identity`, and for Leo/Helga `GET /api/agents/search` + `GET /api/agents/{agentId}` for `systemPrompt`).
